...
public class WizardBodySection extends WizardSection<WizardBodyModel> implements AjaxPageUpdate {
  ...
  @Inject private List<WizardCommand> commands;
  @Inject private List<SectionId> additionalActions;
  ...
  public List<SectionTab> getTabs(SectionInfo info) {
    ...
    return model
        ...
        .get(
            info,
            new CacheFiller<List<SectionTab>>() {
              @Override
  ...
  private List<WizardCommand> getCommands(SectionInfo info) {
    ...
    return model
        ...
        .get(
            info,
            new CacheFiller<List<WizardCommand>>() {
              @Override
  ...
  /**
   * This must run with a priority lower than: {@link PagesSection#submit(SectionInfo)}.
   *
   * @param info
   * @param section
   * @param data
  ...
  public SectionResult renderHtml(RenderEventContext context) throws Exception {
    ...
    model.setAdditionalActions(
        SectionUtils.renderSectionIds(context, additionalActions, new ResultListCollector())
            .getResultList());
    ...
    if ((state.isLockedForEditing() || state.isNewItem())
        && !configurationService.isAutoTestMode()) {
      context
          .getBody()
          .addEventStatements(JSHandler.EVENT_BEFOREUNLOAD, new ReturnStatement(LABEL_WARNING));
    }
